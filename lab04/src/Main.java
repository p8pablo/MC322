...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    clientepf.setValorSeguro(
      seguradoraLab04.calcularPrecoSeguroCliente(clientepf)
    );
    seguradoraLab04.cadastrarCliente(clientepf);

    clientepj.setValorSeguro(
      seguradoraLab04.calcularPrecoSeguroCliente(clientepj)
    );
      ...
      while (opcao != 0) {
        ...
        switch (operacao) {
          ...
          case CALCULAR_RECEITA:
            System.out.println(
              "A receita de " +
              seguradoraLab04.getNome() +
              " é " +
              seguradoraLab04.calcularReceita()
            );
            break;
          case TRANSFERIR_SEGURO: 
            System.out.println("Digite o nome de quem quer fazer a transferência:");
            String nome_transferido = entrada.next();
            for (Cliente clientes : seguradoraLab04.getListaClientes()) {
              if(nome_transferido == clientes.getNome()){
                System.out.println("Digite o nome para quem será transferido:");
                String nome_recebido = entrada.next();
                for(Cliente cliente : seguradoraLab04.getListaClientes()){
                  if(nome_recebido == cliente.getNome()){
                    for(Veiculo veiculos: clientes.getListaVeiculos()){
                      cliente.getListaVeiculos().add(veiculos);
                      clientes.getListaVeiculos().remove(veiculos);
                    }
                  }
                }
                seguradoraLab04.getListaClientes().remove(clientes);
              }
            }
            break;
          case CADASTRAR_PF:
            seguradoraLab04.cadastrarCliente(
          ...
          case CADASTRAR_VEICULO:
            seguradoraLab04
              .getCliente(entrada.next())
              .getVeiculo(Construtor_teclado.criarVeiculo().getPlaca());
  ...
  private static void imprimirMenu(int opcao) {
    ...
    String menuListar =
      "21. Listar Pessoa Física\n 22. Listar Pessoa Jurídica\n 23. Listar Sinistros por Cliente\n 24. Listar Veiculo por Cliente\n25. Listar Veiculo por Seguradora\n 26. Voltar";
    String menuExcluir =
      "31. Excluir Pessoa Física\n 32. Excluir Pessoa Jurídica\n 33. Excluir Sinistro\n 34. Voltar";
    if (opcao == 14 || opcao == 26 || opcao == 34) {
